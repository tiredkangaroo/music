...
2. if you choose not to use docker compose, you're going to need a database in [postgres](https://www.postgresql.org) to connect to. make sure you've already run [schema.sql](https://github.com/tiredkangaroo/music/blob/main/schema.sql) on it. if your postgres is local on port 5432, you can just run the `./resetdb.sh` script.
...
4. set the [environment variables](#environment-variables) necessary. run the binary you just downloaded.
...
| POSTGRES_URL          | required, never if using docker-compose.                                                                       | postgres://musicer:@localhost:5432/music | postgres connection string. docker compose provisions postgres so this should never be specified when using compose. it should link to a database that has the [schema](https://github.com/tiredkangaroo/music/blob/main/schema.sql) already on it. see [this](https://stackoverflow.com/a/20722229/16467184) to help you format a postgres connection url. |
